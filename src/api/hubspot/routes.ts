...
async function getToken(req: Request): Promise<string> {
  try {
    return await hubspotAuth.ensureValidAccessTokenForSession(req)
  } catch {
    const storeKey = req.headers['x-hubspot-storekey'] as string
    if (!storeKey || !hubspotAuth.hubspotTokens.has(storeKey)) {
      throw new Error('No HubSpot authentication - please connect first')
    }
    const store = hubspotAuth.hubspotTokens.get(storeKey)!
...
router.get('/ai-metrics', async (req: Request, res: Response) => {
  try {
    ...
    (data.results || []).forEach((deal: any) => {
      ...
      // AI-powered metrics estimation
      const dealCycleDays = closedAt ? Math.floor((closedAt.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24)) : 30
      const timeSavedHours = Math.round((dealCycleDays * 8) * 0.15) // 15% time savings
      const revenuePullForward = Math.round(forecastAmount * 0.05) // 5% revenue acceleration
    ...
    res.json({
      totalTimeSavedHours,
      totalTimeSavedDays: Math.floor(totalTimeSavedHours / 24),
      totalRevenueImpact,
      dealCount,
