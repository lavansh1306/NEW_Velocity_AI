...
/**
 * Load project health report for a specific project
 */
export async function loadMetrics(projectId: string): Promise<ProjectHealthReport> {
  const issues = await fetchJiraRowsFromApi(projectId);
...
/**
 * Load overall health across all projects.
 */
export async function loadAllMetrics(): Promise<ProjectHealthMetrics> {
  const issues = await fetchJiraRowsFromApi();
...
// getNormalizedEventsForProject removed - no longer using events/normalizers
