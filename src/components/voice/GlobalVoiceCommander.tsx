...
/**
 * GlobalVoiceCommander — UI overlay only.
 * VoiceAgent handles all transcript processing.
 * This component only shows/hides the overlay and handles Ctrl+Space.
 */
export const GlobalVoiceCommander: React.FC = () => {
  ...
  // Keep overlay open while voice is active
  useEffect(() => {
  ...
  // Close overlay when voice goes fully idle
  useEffect(() => {
    if (!isListening && status === 'idle' && isOpen) {
      const timer = setTimeout(() => setIsOpen(false), 1500);
      return () => clearTimeout(timer);
    }
  }, [isListening, status, isOpen]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Status */}
        <p style={{ color: '#1e1b4b', fontSize: 17, fontWeight: 400 }}>{statusLabel}</p>
        ...
        <div className="w-full flex flex-col gap-1.5">
          <p style={{ color: 'rgba(109,40,217,0.4)', fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6, textAlign: 'center' }}>
            Try saying
          </p>
        ...
        <div className="flex items-center gap-3" style={{ color: 'rgba(109,40,217,0.35)', fontSize: 11 }}>
          <span><kbd style={{ padding: '2px 6px', borderRadius: 4, background: 'rgba(139,92,246,0.08)', color: 'rgba(109,40,217,0.5)', fontFamily: 'monospace', fontSize: 10 }}>Esc</kbd> close</span>
          <span style={{ opacity: 0.4 }}>·</span>
          <span><kbd style={{ padding: '2px 6px', borderRadius: 4, background: 'rgba(139,92,246,0.08)', color: 'rgba(109,40,217,0.5)', fontFamily: 'monospace', fontSize: 10 }}>Ctrl+Space</kbd> toggle</span>
        </div>
      </div>
