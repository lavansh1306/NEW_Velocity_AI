export default function HotspotScoringTab() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Task Hotspot Scoring</h2>
        <p className="text-gray-600 mt-1">Ranked by slack, proximity, and Cost of Delay (CoD)</p>
      </div>

      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Filter by Priority</label>
          ...
          <div className="flex-1">
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Team</label>
            <select className="w-full px-4 py-2 border border-gray-300 rounded-lg">
          ...
          <div className="flex-1">
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Time Horizon</label>
            <select className="w-full px-4 py-2 border border-gray-300 rounded-lg">
      ...
      <div className="space-y-4">
        {hotspots.map((hotspot, index) => (
          <div
            key={index}
            className={`bg-white rounded-lg border ${hotspot.borderColor} border-l-4 p-6 hover:shadow-lg transition-all`}
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="font-bold text-gray-900 text-lg mb-1">{hotspot.title}</div>
            ...
            <div className="grid grid-cols-5 gap-4 mb-4">
              <div className="bg-red-50 p-3 rounded-lg">
                ...
                <div className="text-2xl font-bold text-red-600">{hotspot.hotspotScore}</div>
              </div>
              <div className="bg-orange-50 p-3 rounded-lg">
                <div className="text-xs text-gray-600 mb-1">Slack</div>
                <div className="text-lg font-bold text-orange-600">{hotspot.slack}</div>
              </div>
              <div className={`${hotspot.slack.includes('-') ? 'bg-yellow-50' : 'bg-green-50'} p-3 rounded-lg`}>
                <div className="text-xs text-gray-600 mb-1">Cost of Delay</div>
                <div className={`text-lg font-bold ${hotspot.slack.includes('-') ? 'text-yellow-600' : 'text-green-600'}`}>
                  {hotspot.cod}
                </div>
              </div>
              <div className="bg-blue-50 p-3 rounded-lg">
                <div className="text-xs text-gray-600 mb-1">Proximity</div>
                <div className="text-lg font-bold text-blue-600">{hotspot.proximity}</div>
              </div>
              <div className="bg-purple-50 p-3 rounded-lg">
                <div className="text-xs text-gray-600 mb-1">Dependencies</div>
                <div className="text-lg font-bold text-purple-600">{hotspot.dependencies}</div>
              </div>
            </div>
