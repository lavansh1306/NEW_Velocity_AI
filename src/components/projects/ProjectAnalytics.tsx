...
import { useProjectAnalytics, JiraIssue } from '@/hooks/useProjectAnalytics';
...
export default function ProjectAnalytics() {
  ...
  const openEditModal = async () => {
    ...
    try {
      const { data, error } = await supabase.from('projects').select('description, status').eq('id', project.id).single();
  ...
  const handleUpdateProject = async () => {
    ...
    try {
      const { error } = await supabase.from('projects').update({
        name: editForm.title, description: editForm.description, status: editForm.status
      }).eq('id', project.id);
      if (error) throw error;
  ...
  // --- TASK EDIT LOGIC ---
  const openTaskModal = async (issue: JiraIssue) => {
    setIsTaskModalOpen(true);
    setIsTaskFetching(true);
    try {
      const { data, error } = await supabase.from('tasks').select('*').eq('id', issue.id).single();
      if (error) throw error;
      ...
      setTaskForm({
        ...
        assignee_id: data.assignee_id || '', // Empty string means Unassigned
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      const { error } = await supabase.from('tasks').update({
        ...
        assignee_id: taskForm.assignee_id === '' ? null : taskForm.assignee_id,
      ...
      setIsTaskModalOpen(false);
      // No need to reload! The realtime subscription in useProjectAnalytics will automatically catch this and update the UI.
    } catch (e: any) {
              ...
              <>
                ...
                {activeTab === 'team' && (
                  <div className="space-y-8 animate-in fade-in duration-300">
                    {/* ... (Team tab logic remains identical) ... */}
