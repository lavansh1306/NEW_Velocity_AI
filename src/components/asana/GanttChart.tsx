...
export default function GanttChart({ tasks, assignee }: GanttChartProps) {
  ...
  const { stats, tasksByWeek, sortedWeeks, minDate, maxDate, allWeeks } = useMemo(() => {
    ...
    tasks.forEach(task => {
      // use startDate as the primary start; fall back to due if missing
      const taskStart = parseDate(task.startDate) || parseDate(task.due)
      const taskEnd = parseDate(task.due) || taskStart
    ...
    <div className="bg-white rounded-lg shadow-lg p-4 md:p-6 px-8 md:px-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg p-4 border border-amber-200">
          ...
          <p className="text-3xl font-bold text-amber-600">{stats.totalHours}h</p>
        </div>
      ...
      <div className="space-y-6">
        {visibleWeeks.map((weekKey) => {
          ...
          return (
            <div key={weekKey}>
              {/* Week Header */}
              <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg p-4 flex justify-between items-center mb-4">
              ...
              {weekTasks.length > 0 ? (
                <div className="space-y-3">
                  {weekTasks.map((task, idx) => (
                    <div
                      key={idx}
                      className="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:shadow-md hover:border-orange-300 transition-all"
                    >
                      ...
                      <div className="flex gap-4 text-sm text-gray-600">
                        ...
                        <span className="font-semibold text-orange-600">Duration: {task.duration || '-'}d</span>
                      </div>
                    </div>
                  ))}
                </div>
