...
CREATE TABLE public.project_team_allocations (
  ...
  CONSTRAINT project_team_allocations_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id),
  CONSTRAINT project_team_allocations_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
CREATE TABLE public.projects (
  ...
  end_date date,
  created_at timestamp without time zone DEFAULT now(),
  allocated_team_members ARRAY DEFAULT ARRAY[]::uuid[],
  allocated_members_data jsonb DEFAULT '[]'::jsonb,
...
CREATE TABLE public.team_members (
  ...
  user_id uuid,
  role text DEFAULT 'member'::text CHECK (role = ANY (ARRAY['lead'::text, 'member'::text, 'Engineer'::text, 'Designer'::text, 'Product Manager'::text, 'Engineering Manager'::text, 'QA Engineer'::text, 'Data Scientist'::text, 'Frontend Developer'::text, 'Backend Developer'::text, 'Full Stack Developer'::text, 'DevOps Engineer'::text])),
  email text,
  display_name text,
